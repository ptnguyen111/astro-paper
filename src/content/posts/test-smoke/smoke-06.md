---
title: "Test Smoke 06 - Full test 90 minutes"
author: Bob
pubDatetime: 2026-06-01T06:12:00.000+07:00
slug: smoke-06
featured: true
draft: false
tags:
  - test-smoke
  - smoke-06
description: "Bài khảo sát số 6"
---

<figure>
  <img
    src="https://images.pexels.com/photos/7092417/pexels-photo-7092417.jpeg?auto=compress&cs=tinysrgb&w=900"
    alt="A teacher catching a student cheating during an exam in a classroom"
  />
  <figcaption class="text-center">
    Photo by
    <a href="https://www.pexels.com/photo/teacher-caught-his-student-cheating-during-an-exam-7092417/">Pexels</a>
  </figcaption>
</figure>

## Table of contents

## BÀI KHẢO SÁT SỐ 1

_Thời gian làm bài: 30 phút (không kể thời gian phát đề)_  
_THÍ SINH KHÔNG ĐƯỢC PHÉP SỬ DỤNG MÁY TÍNH BỎ TÚI_

### Question 1

> Cholera is a disease that affects the digestive system. People with cholera may experience stomach rumbling, vomiting, and severe diarrhea, which can lead to dehydration and weakness. What is the cause of cholera?

**A**. Bacteria \
**B**. Fungi \
**C**. Virus \
**D**. Yeast

<details name="question-1">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">A — Bệnh tả do vi khuẩn Vibrio cholerae gây ra, thuộc nhóm bacteria (vi khuẩn).</i>
</details>

---

### Question 2

> Air pollution is a major change in the composition of the air. It is mainly caused by smoke, dust, vapor, and harmful gases. It can produce odors, reduce visibility, cause climate change, make people sick, and harm animals or crops. Which of the following images best shows air pollution?
> _[Image A: factory smokestacks emitting thick white smoke]_
> _[Image B: polluted river with trash]_
> _[Image C: plastic waste on beach]_
> _[Image D: dirty water flowing from a pipe]_

**A**. A \
**B**. B \
**C**. C \
**D**. D

<details name="question-2">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">A — Hình A thể hiện ống khói nhà máy thải khói trắng dày đặc — biểu hiện rõ ràng nhất của ô nhiễm không khí.</i>
</details>

---

### Question 3

> In an electric circuit, what is the main function of a wire?

**A**. Supplies electrical energy to the circuit \
**B**. Opens or closes the electrical current \
**C**. Connects parts to let current flow through \
**D**. Uses electricity to do a task

<details name="question-3">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — Dây dẫn có chức năng nối các bộ phận của mạch điện lại với nhau để dòng điện có thể đi qua.</i>
</details>

---

### Question 4

> When growing melons, farmers often use a cotton swab to move pollen from the stamen of the male flower and put it into the stigma of the female flower as shown. Why do they need to do this?

**A**. To make the flower petals grow faster \
**B**. To help the plant absorb more sunlight \
**C**. To allow the plant to produce fruits \
**D**. To protect the flowers from insects

<details name="question-4">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — Thụ phấn giúp cây hình thành quả và hạt.</i>
</details>

---

### Question 5

> Mr. Tony needs to choose a material to make safety helmets for workers to protect them from falling objects. The material must be strong and waterproof. Based on the table, which material is the best choice?

| Material | Strong | Waterproof |
| -------- | ------ | ---------- |
| W        | NO     | YES        |
| X        | YES    | NO         |
| Y        | NO     | YES        |
| Z        | YES    | YES        |

**A**. W \
**B**. X \
**C**. Y \
**D**. Z

<details name="question-5">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — Vật liệu Z có cả hai đặc tính: waterproof = YES và strong = YES.</i>
</details>

---

### Question 6

> Rapunzel wants to test how easily different materials catch fire. In her experiment, she uses a blue flame to test untreated wool and a yellow flame to test flame-proof cotton. What should Rapunzel do to make her experiment a fair test?

**A**. She should test both materials outdoors to see the real effect of wind. \
**B**. She should ensure that she uses the same kind of flame for both materials. \
**C**. She should test more materials to make the experiment more interesting. \
**D**. She should hold the materials at different distances from the flame.

<details name="question-6">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">B — Để thí nghiệm công bằng, chỉ một yếu tố được thay đổi (loại vật liệu). Các yếu tố khác phải giống nhau, bao gồm loại ngọn lửa.</i>
</details>

---

### Question 7

> What is the traditional musical instrument of the Central Highlands (Tây Nguyên)?

**A**. Saxophone \
**B**. Jazz drum \
**C**. Đàn T'rưng \
**D**. Violin

<details name="question-7">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — Đàn T'rưng là nhạc cụ truyền thống của vùng Tây Nguyên, làm từ nứa.</i>
</details>

---

### Question 8

> The Red River Delta region is home to hundreds of long-standing traditional craft villages. The image below shows Vạn Phúc Silk Village, a nationally renowned silk-weaving village that has existed for over a thousand years. In which city is Vạn Phúc Silk Village located today?

**A**. Ninh Bình \
**B**. Hồ Chí Minh \
**C**. Huế \
**D**. Hà Nội

<details name="question-8">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — Làng lụa Vạn Phúc thuộc phường Vạn Phúc, quận Hà Đông, thành phố Hà Nội.</i>
</details>

---

### Question 9

> To fight against the Ming army, in 1418, Lê Lợi started a rebellion in Lam Sơn (Thanh Hóa). Match each phase with its correct major event:
>
> Phase 1 (1418–1423)  
> Phase 2 (1424–1425)  
> Phase 3 (1426–1427)
>
> Events:  
> (a) The army won big battles in Chi Lăng and Xương Giang. The Ming gave up, and the rebellion won.  
> (b) The rebel army was surrounded many times, so they had to move to the mountains of Chí Linh three times.  
> (c) The army freed Nghệ An. The free area grew bigger, from Thanh Hóa to Hải Vân Pass.

**A**. Phase 1 – (a), Phase 2 – (b), Phase 3 – (c) \
**B**. Phase 1 – (b), Phase 2 – (c), Phase 3 – (a) \
**C**. Phase 1 – (c), Phase 2 – (a), Phase 3 – (b) \
**D**. Phase 1 – (a), Phase 2 – (c), Phase 3 – (b)

<details name="question-9">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">B — Giai đoạn 1 (1418-1423): lực lượng yếu, bị bao vây → rút lên núi Chí Linh → (b). Giai đoạn 2 (1424-1425): phản công, giải phóng Nghệ An → (c). Giai đoạn 3 (1426-1427): thắng lớn ở Chi Lăng, Xương Giang → (a).</i>
</details>

---

### Question 10

> The image below shows fighter jets flying over the center of Ho Chi Minh City on the morning of March 28, 2025. These aircraft belong to the Air Defense — Air Force of Vietnam's People's Army. What important national event were these training flights celebrating?
> _[Image: Fighter jets flying over the center of District 1, Ho Chi Minh City (Source: Thanh Nien newspaper)]_

**A**. The 70th anniversary of Vietnam Doctors' Day \
**B**. The 70th anniversary of the Điện Biên Phủ Victory \
**C**. The 50th anniversary of National Reunification \
**D**. The 95th anniversary of the founding of the Communist Party of Vietnam

<details name="question-10">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — 30/4/1975 là ngày Giải phóng miền Nam, thống nhất đất nước. Năm 2025 đánh dấu tròn 50 năm.</i>
</details>

---

### Question 11

> Two squares, each with a side length of 10 cm, overlap as shown in the diagram. The shape of the overlap is also a square, which has an area of 16 square centimetres. In centimetres, what is the perimeter of the combined shape?
> _[Diagram: two overlapping squares, each 10cm side, overlap is a 4cm × 4cm square]_

**A**. 40 cm \
**B**. 56 cm \
**C**. 64 cm \
**D**. 92 cm

<details name="question-11">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — Overlap area = 16cm² → cạnh overlap = 4cm. Chu vi hình kết hợp = 4 × 10 + 4 × 6 = 64 cm.</i>
</details>

---

### Question 12

> Tom has 12 numbers which he wants to put into five cups labeled A, B, C, D, E. He wants the sum of the numbers in each cup to be a whole number. Furthermore, he wants the five whole numbers to be consecutive and increasing from A to E. The twelve numbers Tom has are: 2, 2, 2, 2.5, 2.5, 3, 3, 3, 3, 3.5, 4, and 4.5. If one number 2 goes into cup E and one number 3 goes into cup B, what cup must number 3.5 go into?

**A**. C \
**B**. B \
**C**. D \
**D**. E

<details name="question-12">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — Số 3.5 không thể ở C (7-3.5=3.5, vô lý) hay E (9-2-3.5=3.5, vô lý) → phải ở D (tổng=8).</i>
</details>

---

### Question 13

> The chart below shows the fruits that students in class 3B chose for their picnic. Find the percentage of students choosing bananas or mangoes compared to those choosing apples or strawberries.
> _[Bar chart: Banana=10, Mango=8, Strawberry=12, Apple=8, Orange=6]_

**A**. 80% \
**B**. 125% \
**C**. 15% \
**D**. 90%

<details name="question-13">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — Chuối + Xoài = 10+8 = 18. Dâu + Táo = 12+8 = 20. Tỉ lệ: 18/20 = 90%.</i>
</details>

---

### Question 14

> In class 5A, each desk had only two seats, and no student sat alone. After arranging seats for 40 students, the teacher found that 75% of the boys shared a desk with girls, and 50% of the girls shared a desk with boys. How many girls are there in class 5A?

**A**. 18 girls \
**B**. 16 girls \
**C**. 20 girls \
**D**. 24 girls

<details name="question-14">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — Boy-girl pairs = 75% boys = 3:4, 50% girls = 1:2 = 3:6. Tỉ lệ boys:girls = 4:6 = 2:3. Girls = 3/5 × 40 = 24.</i>
</details>

---

### Question 15

> What does the sign on the right say?
>
> **Susan.**  
> We need to talk urgently about tomorrow's maths test!  
> Give me a ring.  
> — Steve

**A**. Susan needs to phone Steve as soon as possible. \
**B**. Steve expects to call Susan tomorrow. \
**C**. Susan has to contact Steve tomorrow. \
**D**. Steve needs to give Susan a phone call now.

<details name="question-15">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">A — "Give me a ring" và "urgently" → Susan cần gọi Steve sớm nhất có thể (as soon as possible).</i>
</details>

---

### Question 16

> Choose the best answer to fill in the blank.
> A good \_\_\_\_\_\_ is someone who has the skills to manage, guide, and motivate a team of people to complete tasks successfully and effectively.

**A**. explorer \
**B**. worker \
**C**. scientist \
**D**. leader

<details name="question-16">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — Leader = người lãnh đạo, phù hợp mô tả "manage, guide, motivate a team".</i>
</details>

---

### Question 17

> Read the question and choose the best option A, B, C or D. Which of the following sentences is grammatically correct?

**A**. Do you remember the day when we got married that day? \
**B**. Do you still remember the hotel where we had a vacation there last month? \
**C**. The hotel which has friendly staffs it is located on Hàm Nghi Street. \
**D**. The hotel where we stayed during our vacation had a beautiful view.

<details name="question-17">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — Câu D đúng ngữ pháp: dùng "where" làm trạng từ quan hệ chỉ nơi chốn, không thừa từ. Các câu A, B, C đều có lỗi thừa từ (that day, there, it).</i>
</details>

---

### Question 18

> Đọc đoạn thơ sau và cho biết tác giả đã sử dụng những biện pháp tu từ nào?
>
> Trăng bơi trong đáy nước  
> Trăng ngủ trên đỉnh núi  
> Trăng soi giếng khơi trong  
> Trăng lăn dưới đáy sông  
> Trăng sáng ngời lối ngõ  
> Trăng mải soái tìm thú  
> Trăng lăn qua bãi cỏ  
> Trăng cùng em hát múa  
> Trăng vắt cong hàng cây  
> Trăng nghiêng bóng trên đồng  
> Vui đêm rằm Trung thu.

**A**. Điệp từ, nói quá \
**B**. Ẩn dụ, nhân hoá \
**C**. So sánh, điệp từ \
**D**. Điệp từ, nhân hoá

<details name="question-18">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">D — "Trăng" được lặp lại nhiều lần → điệp từ. Trăng bơi, trăng ngủ, trăng soi... → nhân hoá.</i>
</details>

---

### Question 19

> Trong các phương án dưới đây, đâu là phương án sử dụng đúng dấu câu trong tiếng Việt?

**A**. Lớp tôi là một tập thể rất đặc biệt: có bạn học cực kỳ giỏi; có một số bạn khác có nhiều năng khiếu như chơi nhạc cụ, múa, hát... lại có những bạn rất vui tính, lúc nào cũng khiến cả lớp cười ồ lên. \
**B**. Tôi là một đứa... trẻ rất ham chơi: khi cần tập trung học bài thì tôi lại học rất chăm chỉ. \
**C**. Chai nước này có giá bao nhiêu vậy ạ! \
**D**. Quý hỏi tôi rằng có phải tôi có một đứa em không?

<details name="question-19">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">A — A: dùng đúng dấu hai chấm (liệt kê), dấu chấm phẩy (tách nội dung phức tạp), dấu ba chấm (báo hiệu phần chưa nói hết).</i>
</details>

---

### Question 20

> Câu nào dưới đây thể hiện đúng mối quan hệ giả thiết – kết quả giữa các vế câu trong câu ghép?

**A**. Giá mà tôi không ương bướng thì cái Linh đã chẳng giận tôi đến thế. \
**B**. Nếu ngày mai bão đổ bộ đất liền mà gia đình tôi sẽ rời đi nơi khác. \
**C**. Nước là tài nguyên có hạn nên chúng ta cần phải tiết kiệm nước. \
**D**. Nếu cần mẫn làm lụng, hắn ta đã có của ăn của để chứ không vướng vào vòng lao lý như bây giờ.

<details name="question-20">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">A — "Giá mà... thì..." là cặp kết từ thể hiện đúng quan hệ giả thiết - kết quả.</i>
</details>

---

**— HẾT BÀI KHẢO SÁT SỐ 1 —**

---

## BÀI KHẢO SÁT SỐ 2 — PHẦN TIẾNG ANH

_Thời gian làm bài 3 phần: 60 phút (không kể thời gian phát đề)_

<figure>
  <img
    src="https://images.pexels.com/photos/6684211/pexels-photo-6684211.jpeg?auto=compress&cs=tinysrgb&w=900"
    alt="A student writing answers on a multiple-choice exam sheet at a desk while cheating"
  />
  <figcaption class="text-center">
    Photo by
    <a href="https://www.pexels.com/photo/a-person-writing-on-paper-while-cheating-6684211/">Andy Barbour</a>
    on Pexels
  </figcaption>
</figure>

<!-- PART 1: LISTENING — Skipped as per instruction -->

### PART 2: READING (16 points)

_Read the following passage and answer the questions._

**GROWTH AND FIXED MINDSET**

When we talk about mindsets, we often hear the terms "growth" and "fixed". Understanding the differences between these two mindsets is important for students. A "growth mindset" is when people believe they can improve their skills and intelligence through hard work and dedication. For example, a student with a growth mindset will often try out new study methods when they find a subject challenging. They see failures as opportunities to learn, and this can lead to greater achievements in school and life.

On the other hand, a fixed mindset means that a person believes their skills and talents are permanent and cannot be changed. This means they often shy away from challenges and may give up easily when things get harder or tougher. While having a fixed mindset can give a sense of comfort and security, it often limits personal growth and can lead to feelings of tiredness. Students with a fixed mindset might say, "I'm just not good at math," and stop trying, missing the chance to improve their skills.

In conclusion, embracing a growth mindset can provide many advantages, such as improving learning experiences and helping students deal with challenges in a more positive way. However, it is also necessary to recognize that a fixed mindset can also have some advantages, like feeling less pressure to be perfect. Knowing these differences can help students choose which mindset they want to adopt in their learning journey.

---

#### Question 1

> Which of the following statements can best describe the term "growth mindset"?
> (Write A, B, or C in the blank)

**A**. Believing that abilities cannot be changed \
**B**. Believing everyone can become better through hard work \
**C**. Believing talents are something a person cannot have

<details name="question-1">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">B — Believing everyone can become better through hard work</i>
</details>

---

#### Question 2

> How do students with a growth mindset think about failures?
> (Write A, B, or C in the blank)

**A**. As reasons to give up \
**B**. As bad things to face \
**C**. As opportunities to learn and improve

<details name="question-2">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">C — As opportunities to learn and improve</i>
</details>

---

#### Question 3

> How do people with a fixed mindset feel about new challenges?
> (Write A, B, or C in the blank)

**A**. They readily deal with challenges to learn more. \
**B**. They often avoid challenges and give up quickly. \
**C**. They face challenges without any worries.

<details name="question-3">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">B — They often avoid challenges and give up quickly</i>
</details>

---

#### Question 4

> Decide whether this statement is True or False:
> "The passage indicates that people with a growth mindset will give up easily when things are difficult."

(Write True or False in the blank)

<details name="question-4">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">False — Passage says growth mindset people DO NOT give up easily</i>
</details>

---

#### Question 5

> Decide whether this statement is True or False:
> "Students with a fixed mindset don't feel stressed and exhausted when trying new ways of studying."

(Write True or False in the blank)

<details name="question-5">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">False — Fixed mindset people feel stressed (the passage says "feelings of tiredness")</i>
</details>

---

#### Question 6

> Decide whether this statement is True or False:
> "The passage implies that recognizing the differences between the two mindsets can assist students in the learning process."

(Write True or False in the blank)

<details name="question-6">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">True — "Knowing these differences can help students choose which mindset they want to adopt"</i>
</details>

---

#### Question 7

> What is one of the benefits a student may have if they have a growth mindset?

(Write your answer in the blank)

<details name="question-7">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Improving learning experiences / helping students deal with challenges in a more positive way — Chấp nhận cách diễn đạt khác nhưng giống thông tin.</i>
</details>

---

#### Question 8

> Find ONE word in the passage that means "the hard work and effort that somebody puts into an activity because they think it is important."

(Write your answer in the blank)

<details name="question-8">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">dedication — Không cho điểm nếu sai chính tả</i>
</details>

---

### PART 3: WRITING (6 points)

**Task 1: Rearrange the words below to make a meaningful sentence.**

_Example:_

> badminton / great / player / Anna / a / is //
> → Anna is a great badminton player.

#### Question 1

> badminton / off / to / with / the TV / play / turned / Anna / her friends //
> → \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

<details name="question-1">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Anna turned the TV off to play badminton with her friends.</i>
</details>

---

**Task 2: Write complete sentences with the given cues. You must write NO MORE THAN fifteen words for each sentence.**

_Example:_

> Tuan / be / tall / Anna //
> → Tuan is taller than Anna.

#### Question 2

> Lucy / her friends / shop / mall / the moment //
> → \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

<details name="question-2">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Lucy and her friends are shopping at the mall at the moment.</i>
</details>

---

#### Question 3

> every year / Vietnamese people / occasion / call / "Independence Day" / remember / country / freedom //
> → \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

<details name="question-3">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Every year, Vietnamese people celebrate an occasion called "Independence Day" to remember the country's freedom.</i>
</details>

---

**— KẾT THÚC PHẦN TIẾNG ANH —**

---

## PHẦN TOÁN & IQ

| PART 1 | PART 2 | PART 3 | FINAL |
| ------ | ------ | ------ | ----- |
|        |        |        |       |

_Giám thị không giải thích gì thêm_

### Câu 1 (3 điểm)

> Lễ kỉ niệm 50 năm Ngày Giải phóng miền Nam, thống nhất đất nước được tổ chức vào thứ Tư, ngày 30/4/2025. Hỏi lễ kỉ niệm 60 năm Ngày Giải phóng miền Nam, thống nhất đất nước là ngày thứ mấy trong tuần? **Chỉ viết kết quả.**

Trả lời: \_\_\_\_\_\_\_\_\_\_\_\_\_\_

<details name="question-1">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Thứ Hai</i>
</details>

---

### Câu 2 (4 điểm)

> Một cửa hàng hoa quả có một số táo và cam. Sau khi bán cùng số lượng táo và cam, cửa hàng còn lại 20 quả cam và 60 quả táo. Hỏi người đó đã bán được bao nhiêu quả cam, biết ban đầu số quả cam bằng 3/5 số quả táo?
>
> **Trình bày lời giải, phép tính, đáp số.**

<details name="question-2">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">40 quả cam. Hiệu số táo và cam ban đầu = sau khi bán = 60 − 20 = 40 (quả). Ban đầu cam = 3/5 táo → giá trị 1 phần: 40 ÷ (5 − 3) = 20 (quả). Số cam ban đầu: 20 × 3 = 60 (quả). Số cam đã bán: 60 − 20 = 40 (quả).</i>
</details>

---

### Câu 3 (8 điểm)

> Nhân đi xe đạp từ nhà (điểm A) đến trường (điểm C) lúc 5 giờ. Đoạn đường từ A đến C gồm một đoạn đường bằng phẳng (AB) và một đoạn đường dốc (BC).
>
> - Trên đoạn đường bằng phẳng AB, Nhân đi với vận tốc 4 km/giờ.
> - Trên đoạn đường dốc BC, Nhân đi với vận tốc 6 km/giờ.
> - Nhân đến trường lúc 6 giờ 48 phút, và nghỉ dọc đường 48 phút.
>
> a) Tính thời gian Nhân di chuyển trên đoạn đường bằng phẳng.  
> b) Biết AB = BC, tính độ dài quãng đường từ nhà Nhân đến trường.
>
> **Trình bày lời giải, phép tính, đáp số.**

<details name="question-3">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">a) 0.6 giờ (36 phút) — Thời gian từ 5h → 6h48: 1 giờ 48 phút. Trừ thời gian nghỉ: 1h48 − 48 phút = 1 giờ. Tỉ số vận tốc AB:BC = 4:6 = 2:3. AB = BC → tỉ số thời gian AB:BC = 3:2. Thời gian AB: 1 giờ × 3/5 = 0.6 giờ = 36 phút. b) 4.8 km — Độ dài AB: 0.6 × 4 = 2.4 km. BC = AB = 2.4 km. AC = 2.4 × 2 = 4.8 km.</i>
</details>

---

**— KẾT THÚC PHẦN TOÁN & IQ —**

---

## PHẦN ĐỌC HIỂU & LÀM VĂN

| PART 1 | PART 2 | PART 3 | FINAL |
| ------ | ------ | ------ | ----- |
|        |        |        |       |

_Giám thị không giải thích gì thêm_

### Câu 1. Đọc hiểu (8 điểm)

Đọc đoạn thơ sau và trả lời các câu hỏi.

**HÀNH TINH NHỎ**

_Khi em bé vừa chào đời_  
_Nụ cười mẹ là ánh sáng_  
_Đôi tay cha là bầu trời_  
_Câu chuyện của bà là mặt đất_  
_Ông là dòng nước mát trong_

_Hành tinh nhỏ của ông bà_  
_Hành tinh nhỏ của cha mẹ_  
_Mỗi ngày một điều đổi khác_  
_Thêm hoa thêm cỏ xanh rì_

_Và rồi một ngày thức giấc_  
_Cha mẹ vội vã ra đi_  
_Ông bà ru em ngủ tiếp_  
_Hành tinh nhỏ ngủ thật ngoan_

_(Theo Nguyễn Hữu Thông)_

> **a) (2 điểm)** Trong bài thơ, khi em bé vừa chào đời, mỗi người trong gia đình được liên tưởng với những hình ảnh khác nhau. Hãy cho biết những hình ảnh đó là gì.

<details name="question-1-a">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Nụ cười mẹ: ánh sáng; Đôi tay cha: bầu trời; Câu chuyện của bà: mặt đất; Ông: dòng nước mát. Mỗi ý 0.5đ. Phải ghi rõ hình ảnh (vd: "nụ cười mẹ - ánh sáng").</i>
</details>

---

> **b) (2 điểm)** "Hành tinh nhỏ" trong bài thơ này là hình ảnh của ai? Sự "đổi khác" mỗi ngày của "hành tinh nhỏ" nói lên điều gì?

<details name="question-1-b">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">"Hành tinh nhỏ" là đứa bé vừa chào đời. Sự "đổi khác" mỗi ngày là quá trình lớn lên và thay đổi của em bé. Đủ 2 ý, mỗi ý 1đ.</i>
</details>

---

> **c) (2 điểm)** Khổ thơ thứ ba miêu tả điều gì?

<details name="question-1-c">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Khổ thơ thứ ba miêu tả cuộc sống hàng ngày trong gia đình: cha mẹ bận rộn vì công việc, ông bà chăm sóc em bé.</i>
</details>

---

> **d) (2 điểm)** Em hãy nêu cảm nhận của mình về thông điệp mà bài thơ muốn gửi gắm.

<details name="question-1-d">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Bài thơ nhấn mạnh: trẻ em khi sinh ra thuần khiết như một hành tinh mới; cần được yêu thương, chăm sóc của gia đình.</i>
</details>

---

### Câu 2. Làm văn (7 điểm)

> Hãy viết một đoạn văn (khoảng 15–20 dòng) trình bày cảm nhận của em về bài thơ "Hành tinh nhỏ" của tác giả Nguyễn Hữu Thông.

<details name="question-2">
    <summary><i>Show awnswer key</i></summary>    
    <i class="answer-key">Rubric chấm: Format (2đ) — Viết hoa đầu đoạn, đầu câu; lùi đầu đoạn; dấu câu hợp lý. Content (4đ) — Giới thiệu tên bài thơ (0.25đ), tác giả (0.25đ), ấn tượng chung (0.25-0.5đ). Khái quát nội dung (0.5đ). Cảm nhận, đánh giá nội dung + dẫn chứng (0.5đ + 0.5đ). Cảm nhận sâu sắc về nghệ thuật (biện pháp tu từ — 1đ). Creativity (1đ) — Có suy nghĩ riêng, diễn đạt mới mẻ, không rập khuôn.</i>
</details>

---

**— KẾT THÚC PHẦN ĐỌC HIỂU & LÀM VĂN —**

---

**— HẾT —**
